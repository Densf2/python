...
def diff():
    # +++ ваш код +++
    square = reduce(operator.add, [i*i for i in xrange(1, 100+1)])

    squared = reduce(operator.add, xrange(1, 100+1))

    multi_square = squared * squared
    return multi_square - square
...
def max_five():
    ...
    digits = []
    while c <= len(n) - 5:
        digits.append(n[c:c + 5])
        c += 1

    biggest = 0
    for d in digits:
        product = int(d[0]) * int(d[1]) * int(d[2]) * int(d[3]) * int(d[4])
        if product > biggest:
            biggest = product
